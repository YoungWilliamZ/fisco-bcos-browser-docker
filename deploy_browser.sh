...
dbIp="db"   #数据库所在机器IP
tomcatpath="${CATALINA_HOME}" #tomcat所在路径
...
#rm ${deployserverpath}/bcos-server* -rf
#cp "${serverpath}/bcos-server/apps/bcos-server.war" "${deployserverpath}"
...
#rm ${deployserverpath}/fisco-bcos-browser* -rf
#cp "${pagepath}/dist/apps/fisco-bcos-browser.war" "${deployserverpath}"
...
set +e && sh shutdown.sh && set -e

rm ${deployserverpath}/fisco-bcos-server* -rf
cp "${serverpath}/fisco-bcos-server/apps/fisco-bcos-server.war" "${deployserverpath}"
